...
procedure TfrmiTableNd.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfrmiTableNd.InitForEdit(InParam: PTParamForm);
begin
  FInParam := InParam;
...
procedure TfrmiTableNd.InitForNew(InParam: PTParamForm);
begin
  FInParam := InParam;
...
procedure TfrmiTableNd.LoadDataFBase;
  var
    IBSQLG1: TIBSQL;
begin
...
procedure TfrmiTableNd.EditSTB(Sender: TObject);
  var
   IBTable1: TIBTable;
begin
...
procedure TfrmiTableNd.NewSTB(Sender: TObject);
  var
   IBTable1: TIBTable;
begin
  IBTable1 := TIBTable.Create(nil);
...
function TfrmiTableNd.TestOnRight: boolean;
begin
  Result := true;
...
procedure TfrmiTableNd.btOKClick(Sender: TObject);
begin
...
procedure TfrmiTableNd.btCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmiTableNd.FormDestroy(Sender: TObject);
begin
  ForDestroyForm;
end;

procedure TfrmiTableNd.ForDestroyForm;
begin
